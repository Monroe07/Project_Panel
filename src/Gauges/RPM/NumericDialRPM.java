...
public class NumericDialRPM {
     ...
     * @param _dialDiameter
     */
	NumericDialRPM(int fontsize, int _dialDiameter) {
		dispFont = new Font("SansSerif", Font.BOLD, fontsize);
		...
		maxValue = 10; /// Was 10
		minValue = 0;
		bigLineStep = 1;		// Sets Spacing of small lines on outside of Gauges.RPM.RPM
	...
	void draw(Graphics g) {
		...
		for (double i = minValue; i <= maxValue; i += bigLineStep) {
            ...
            if (i >= maxValue) break; // Checks for max value and leaves for loop id >= to maxValue
            //draw sub lines
